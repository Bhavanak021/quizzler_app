...
class QuizInterface:
    ...
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.question_text = self.canvas.create_text(150, 120, width=280, text="Hello", font=("Arial", 20, "italic"), fill=THEME_COLOR)
        self.canvas.grid(row=1, column=0, columnspan=2, pady=50)

        self.true_img = PhotoImage(file="./images/true.png")
        self.false_img = PhotoImage(file="./images/false.png")
        self.true_btn = Button(image=self.true_img, highlightthickness=0, bg=THEME_COLOR, command=self.true_pressed)
        self.true_btn.grid(row=3, column=0)
        self.false_btn = Button(image=self.false_img, highlightthickness=0, bg=THEME_COLOR, command=self.false_pressed)
